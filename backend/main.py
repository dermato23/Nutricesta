...
def generate_recipes_background(receipt_id: int, raw_text: str):
    ...
    db = database.SessionLocal()
    try:
        recipes_list = ai_parser.generate_recipes_with_gemini(raw_text)
        if recipes_list:
            import json
...
def upload_receipt(request: schemas.OCRRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    db_receipt = models.Receipt(
        store_name="Desconocido", # Debería extraerse del OCR
...
def get_financial_stats(year: int = None, month: int = None, db: Session = Depends(get_db)):
    ...
    ALL_CATEGORIES = [
        ...
        "Carbohidratos", "Cereales", "Granos", "Productos de Aseo Personal", 
        "Aseo de Hogar", "Snacks o Antojos", "Mascotas"
    ]
